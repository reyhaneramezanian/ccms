...
import { useUser_GetActiveUsersQuery } from 'src/graphql/generated';
...
const ChartBaruser = () => {
    ...
    const date = new Date();
    const { data: dataBar } = useUser_GetActiveUsersQuery({
        skip: 0,
        take: totalqty,
        where:
    ...
    if (loop === 0 && dataBar?.user_getActiveUsers?.result?.totalCount !== undefined) {
        setloop(1);
        settotalqty(dataBar?.user_getActiveUsers?.result?.totalCount);
    ...
    useEffect(() => {
        ...
        var month = [],
            ...
            day = [],
            year = [];
        dataBar?.user_getActiveUsers?.result?.items?.map((item) => {
            ...
            if (year.length > 0) {
                year.forEach((v, i) => {
                    if (v.month === dt.getMonth() + 1) {
                        var Qty = v.Qty + item.activeUserCount;
                        js.slice(1, i);
                ...
                else year.push({ month: dt.getMonth() + 1, Qty: item.activeUserCount });
            } else year.push({ month: dt.getMonth() + 1, Qty: item.activeUserCount });

            day.push({ day: dt.getDay(), Qty: item.activeUserCount });
            month.push({ day: dt.getDate(), Qty: item.activeUserCount });
        });
        ...
        <Grid container>
            ...
            <Grid item md={12} xs={12} lg={12} sm={12}>
                <adminstyle.imgcontainer>
                    <adminstyle.chartbids>User</adminstyle.chartbids>
