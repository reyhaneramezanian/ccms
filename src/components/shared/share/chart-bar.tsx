...
import { useBid_GetBidsQuery } from 'src/graphql/generated';
...
const ChartBar = () => {
    ...
    const date = new Date();
    const { data: dataBar } = useBid_GetBidsQuery({
        skip: 0,
        take: totalqty,
        where:
    ...
    if (loop === 0 && dataBar?.bid_getBids?.result?.totalCount !== undefined) {
        setloop(1);
        settotalqty(dataBar?.bid_getBids?.result?.totalCount);
    ...
    useEffect(() => {
        var month = [],
            ...
            day = [],
            year = [];
        dataBar?.bid_getBids?.result?.items?.map((item) => {
            var dt = new Date(item?.createdDate);
            year.push(dt.getMonth() + 1);
            day.push(dt.getDay());
            month.push(dt.getDate());
        });
        ...
        <Grid container>
            ...
            <Grid item md={12} xs={12} lg={12} sm={12}>
                <adminstyle.imgcontainer>
                    <adminstyle.chartbids>Bids</adminstyle.chartbids>
