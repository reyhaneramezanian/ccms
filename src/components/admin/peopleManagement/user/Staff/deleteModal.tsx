...
import { useUser_DeleteStaffMutation } from 'src/graphql/generated';
...
const DeleteModal: FC<IModalBodyProps<RowTable>> = ({ data: { rowIds, refetch } }) => {
    ...
    const { mutate, isLoading } = useUser_DeleteStaffMutation();
            ...
            rowIds.map((item) =>
                mutate(
                    { staffId: item },
                    {
                        ...
                        onError: (err) => {
                            mutationErrorHandler(err, 'user_deleteStaff');
                        }
                    }
                )
            )
