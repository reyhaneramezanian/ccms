...
import { useUser_DeleteSecurityMutation } from 'src/graphql/generated';
...
const DeleteModal: FC<IModalBodyProps<RowTable>> = ({ data: { rowIds, refetch } }) => {
    ...
    const { mutate, isLoading } = useUser_DeleteSecurityMutation();
            ...
            rowIds.map((item) =>
                mutate(
                    { securityId: item },
                    {
                        ...
                        onError: (err) => {
                            mutationErrorHandler(err, 'user_deleteSecurity');
                        }
                    }
                )
            )
