...
const PeaplemanagementAddModal: FC<IModalBodyProps<RowTable>> = ({ data: { row, refetch } }) => {
    ...
    const { data: dataResident } = useResidentFlat_GetResidentFlatsQuery({
        take: totalbuilding?.residentFlat_getResidentFlats?.result?.totalCount,
        where: {
            flat: {
                floor: {
                    blockId: {
    ...
    const handelsave = (e) => {
        mutate(
            ...
            },
            {
                ...
                onError: (err) => {
                    mutationErrorHandler(err, 'blockManager_update');
                }
            }
        );
    };
