...
import { useBlockManager_DeleteMutation } from 'src/graphql/generated';
...
const DeleteModal: FC<IModalBodyProps<RowTable>> = ({ data: { rowIds, refetch } }) => {
    ...
    const { mutate, isLoading } = useBlockManager_DeleteMutation();
            ...
            rowIds.map((item) =>
                mutate(
                    { id: item },
                    {
                        ...
                        onError: (err) => {
                            mutationErrorHandler(err, 'blockManager_delete');
                        }
                    }
                )
            )
