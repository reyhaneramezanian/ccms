...
import { useUser_DeleteComplexManagerMutation } from 'src/graphql/generated';
...
const DeleteModal: FC<IModalBodyProps<RowTable>> = ({ data: { rowIds, refetch } }) => {
    ...
    const { mutate, isLoading } = useUser_DeleteComplexManagerMutation();
            ...
            rowIds.map((item) =>
                mutate(
                    { entityId: item },
                    {
                        ...
                        onError: (err) => {
                            mutationErrorHandler(err, 'user_deleteComplexManager');
                        }
                    }
                )
            )
