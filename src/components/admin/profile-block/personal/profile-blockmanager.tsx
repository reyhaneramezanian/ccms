...
const Profile = () => {
    ...
    const { data: datauserblock } = useBlockManager_GetBlockManagersQuery();
    useEffect(() => {
        let complexname = '';
        setgender(datauser?.user_getCurrentResident?.result?.gender);
        setactiveStatu(datauser?.user_getCurrentResident?.result?.activeStatus);
        datauserblock?.blockManager_getBlockManagers?.result?.items?.forEach((item, index) => {
            complexname = item.block.name + ' ,' + complexname;
        });
    ...
    const handelsave = (e) => {
        mutate(
            {
                input: {
                    //externalId: datauser?.user_getCurrentResident?.result?.externalId,
                    gender: e.Gender,
    ...
    return (
        <adminstyle.containerprofile>
            <adminstyle.profileheader>
                ...
                <adminstyle.profilenameuser>{pageData.fullnameprofile}</adminstyle.profilenameuser>
            </adminstyle.profileheader>
