...
const BuildingConfigurationAddModal: FC<IModalBodyProps<RowTable>> = ({ data }) => {
    ...
    const { mutate, isLoading } = useFlat_CreateMutation();
    ...
    const [dataselectblock, setdataselectblock] = useState(0);
    ...
    useEffect(() => {
        ...
        dataBlock?.block_getBlocks?.result?.items.map((item) => {
            jsblock.push({ option: item.name, value: item.id, index: indexadd });
        });
        dataFloor?.floor_getFloors?.result?.items.map((item) => {
            jsfloor.push({ option: item.name, value: item.id, index: indexadd });
        });
        ...
        blocklist.forEach((item, i) => {
            if (i === indexadd) {
                ...
                if (item.length > 0) jsblocklist.push(item);
            }
        });
        ...
        floorlist.forEach((item, i) => {
            if (i === indexadd) {
                jsfloorlist.push(jsfloor);
                sw = 1;
            } else {
                if (item.length > 0) jsfloorlist.push(item);
            }
        });
        if (sw == 0) jsfloorlist.push(jsfloor);
        setfloorlist(jsfloorlist);
        setcomplexlist(js);
        setblocklist(jsblocklist);
    }, [datacomplex, dataBlock, dataFloor]);
            ...
            <>
                ...
                <Select
                    ...
                    onChange={(e) => {
                        ...
                        setcomplexitem(js);
                        setdataselect(Number(e.target.value));
                    }}
                    IconComponent={() => <Down />}
                    variant="outlined">
                    {options.map((item) => (
    ...
    const BlockSelect = ({ name, options, label, indexrow }) => {
        const [field, meta, helpers] = useField(name);
        setindexadd(indexrow);
        ...
        return (
            <>
                ...
                <Select
                    ...
                    value={valueblock(indexrow)}
                    onChange={(e) => {
                        var js = [],
                            sw = 0,
                            repete = 0;
                        blockitem.forEach((item, i) => {
                            if (item.index === indexrow) {
                                js.push({ value: e.target.value, index: item.index });
                                sw = 1;
                            } else js.push(item);
                        ...
                        if (repete + 1 === indexrow) {
                            var jsblock = floorlist;
                            floorlist?.forEach((item, i) => {
                                if (i === repete) jsblock.push(item);
                            });
                            setfloorlist(jsblock);
                        }

                        if (sw === 0) js.push({ value: e.target.value, index: indexrow });
                        setindexadd(indexrow);
                        setblockitem(js);
                        setdataselectblock(Number(e.target.value));
                    }}
                    IconComponent={() => <Down />}
                    variant="outlined">
                    {options.map((item) => (
            ...
            e.listComplex.map((item) =>
                mutate(
                    ...
                    },
                    {
                        ...
                        onError: (err) => {
                            mutationErrorHandler(err, 'flat_create');
                        }
                    }
                )
            )
