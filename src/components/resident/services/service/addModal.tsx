...
import { Typography, Grid, Checkbox, Box, FormControlLabel, MenuItem, Select } from '@mui/material';
...
const ServiceAddModal: FC<IModalBodyProps<RowTable>> = ({ data }) => {
    ...
    const handleCancel = () => {
        dispatch(closeModal(ServiceAddModal.name));
    };
    const handelsave = (e) => {
        mutate(
            {
...
const handleShowServiceAddModal = (data?: RowTable) => {
    return newModal({
        Body: ServiceAddModal,
        title: 'Add request service',
        topBar: true,
        id: ServiceAddModal.name,
        data
    });
};

export default handleShowServiceAddModal;
