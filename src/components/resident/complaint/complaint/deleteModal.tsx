...
import { useComplaint_DeleteMutation } from 'src/graphql/generated';
...
const ComplaintDeleteModal: FC<IModalBodyProps<RowTable>> = ({ data: { rowIds, refetch } }) => {
    ...
    const { mutate, isLoading } = useComplaint_DeleteMutation();
            ...
            rowIds.map((item) =>
                mutate(
                    { entityId: item },
                    {
                        ...
                        onError: (err) => {
                            mutationErrorHandler(err, 'complaint_delete');
                        }
                    }
                )
            )
